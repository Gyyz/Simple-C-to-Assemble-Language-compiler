...
void initOperMapping()//算数运算以及布尔运算符
{
    operMap.clear();
...
void createNewNode(char * content,char *descirbe,int type,int addr,int line)//Token节点
{
...
void createNewError(char * content,char *descirbe,int type,int line)//错误节点
{
...
void printNodeLink()
{
    NormalNode * p = normalHead;//Token
    p = p->next;
	ofstream LexAnl;
	LexAnl.open("LexAnl.txt",ios::out);
    cout<<"************************************分析表******************************"<<endl<<endl;
	LexAnl<<"************************************分析表******************************"<<endl;
	//LexError.write("");
   // LexError.close();
	cout<<setw(30)<<"内容"<<setw(10)<<"描述"<<"\t"<<"种别码"<<"\t"<<"地址"<<"\t"<<"行号"<<endl;
	LexAnl<<setw(5)<<"内容"<<setw(10)<<"描述"<<"\t"<<"种别码"<<"\t"<<"地址"<<"\t"<<"行号"<<endl;
    while(p!=NULL)
    {   
        if(p->type == IDENTIFER)
        {
            cout<<setw(30)<<p->content<<setw(10)<<p->describe<<"\t"<<p->type<<"\t"<<p->addr<<"\t"<<p->line<<endl;
			LexAnl<<setw(5)<<p->content<<setw(10)<<p->describe<<"\t"<<p->type<<"\t"<<p->addr<<"\t"<<p->line<<endl;
        }
        else
        {
...
void printErrorLink()
{
    ErrorNode * p = errorHead;
	ofstream LexError;
	LexError.open("LexError.txt",ios::out);
    p = p->next;
    cout<<"************************************错误表******************************"<<endl<<endl;
    cout<<setw(10)<<"内容"<<setw(30)<<"描述"<<"\t"<<"类型"<<"\t"<<"行号"<<endl;
	LexError<<"************************************错误表******************************"<<endl<<endl;
	LexError<<setw(10)<<"内容"<<setw(30)<<"描述"<<"\t"<<"类型"<<"\t"<<"行号"<<endl;
    while(p!=NULL)
    {
        cout<<setw(10)<<p->content<<setw(30)<<p->describe<<"\t"<<p->type<<"\t"<<p->line<<endl;
		LexError<<setw(10)<<p->content<<setw(30)<<p->describe<<"\t"<<p->type<<"\t"<<p->line<<endl;
        p = p->next;
    }
	LexError.close();
    cout<<endl<<endl;
...
void printIdentLink()
{
    IdentiferNode * p = idenHead;
	ofstream LexIdent;
	LexIdent.open("LexIdent.txt",ios::out);
    p = p->next;
    cout<<"************************************标志符表******************************"<<endl<<endl;
	LexIdent<<"************************************标志符表******************************"<<endl<<endl;
    cout<<setw(30)<<"内容"<<setw(10)<<"描述"<<"\t"<<"种别码"<<"\t"<<"地址"<<"\t"<<"行号"<<endl;
	LexIdent<<setw(30)<<"内容"<<setw(10)<<"描述"<<"\t"<<"种别码"<<"\t"<<"地址"<<"\t"<<"行号"<<endl;
    while(p!=NULL)
    {
        cout<<setw(30)<<p->content<<setw(10)<<p->describe<<"\t"<<p->type<<"\t"<<p->addr<<"\t"<<p->line<<endl;
		LexIdent<<setw(30)<<p->content<<setw(10)<<p->describe<<"\t"<<p->type<<"\t"<<p->addr<<"\t"<<p->line<<endl;
        p = p->next;
    }
    cout<<endl<<endl;
	LexIdent.close();
...
int seekKey(char * word)  //查看是否为关键字
{
...
void scanner()
{
    ...
    while(ch!=EOF)
    {
        ...
        if((ch>='A' && ch<='Z') || (ch>='a' && ch<='z') || ch == '_')
        {
            ...
            word = new char[i+1];
            memcpy(word,array,i);  //拷贝单词
            word[i] = '\0';
            int seekTemp = seekKey(word); //临时变量
