...
int seekCodeNum(char * word)
{
    ...
    for(int i = 0; i<specialMap.size(); i++)
    {
        if(strcmp(word,specialMap[i].first) == 0)//相同
        {
...
void initGrammer()
{
    ...
    memset(proc,-1,sizeof(proc));//重置数组前sizeof大小为-1
    ...
    while(ch!=EOF)
    {
        ...
        if(codeNum!=0)
        {
            ...
            proc[line][count] = codeNum;//存放产生式，每个数组空间存放一个推导式
    ...
    printf("\n************************************文法终结符******************************\n\n");
	SynTer.open("SynTer.txt",ios::out);
	SynTer<<"************************************文法终结符******************************"<<"\n"<<endl;
	SynTer<<"编号："<<"\t\t"<<"终结符"<<endl<<endl;
    for(int i=0; i<terMap.size(); i++)
    {   
        printf("%s\t",terMap[i].first);
		SynTer<<i<<"\t\t"<<terMap[i].first<<endl;
    }
    printf("\n");
    printf("\n************************************文法非终结符******************************\n\n");
	SynNTer.open("SynNTer.txt",ios::out);
	SynNTer<<"************************************文法非终结符******************************"<<"\n"<<endl;
	SynNTer<<"编号："<<"\t\t"<<"非终结符"<<endl<<endl;
    for(int i=0; i<nonTerMap.size(); i++)
    {
        printf("%s\t\t",nonTerMap[i].first);
		SynNTer<<i<<"\t\t"<<nonTerMap[i].first<<endl;
    }
    printf("\n");
	SynTer.close();
	SynNTer.close();
...
void First()
{
    //先求出能直接推出空的非终结符集合
	ofstream SynFirst;
	SynFirst.open("SynFirst.txt",ios::out);
    ...
    for(int i=0; i<nonTerMap.size(); i++)
    {
		...
		SynFirst<<"First["<<nonTerMap[i].first<<"] =";
        for(int j=0;; j++)
        {
            if(first[i][j] == -1)
            {
			...
			SynFirst<<setw(5)<<searchMapping(first[i][j])<<setw(5);
        }
        printf("\n");
		SynFirst<<endl;
    }
...
{
	ofstream SynFollow;
	SynFollow.open("SynFollow.txt",ios::out);
    for(int i=0; i<nonTerMap.size(); i++)
    {
        followRecu[0] = -1;
        followSet(i);
    ...
    printf("\n************************************Follow集******************************\n\n");
	SynFollow<<"************************************Follow集******************************"<<endl;
    for(int i=0; i<nonTerMap.size(); i++)
    {
        printf("Follow[%s] = ",nonTerMap[i].first);
		SynFollow<<"Follow["<<nonTerMap[i].first<<"] =";
        for(int j=0;; j++)
        {
            if(follow[i][j] == -1)
            {
			...
			SynFollow<<setw(5)<<searchMapping(follow[i][j])<<setw(5);
        }
        printf("\n");
		SynFollow<<endl;
    }
...
{
	ofstream SynSelect;
	SynSelect.open("SynSelect.txt",ios::out);
    ...
    for(int i=0; i<procNum; i++)
    {
        ...
        for(int j=0;; j++)
        {
			...
			SynSelect<<setw(5)<<searchMapping(select[i][j])<<setw(5);
        }
        printf("\n");
		SynSelect<<endl;
    }
...
void Analysis()
{
    ...
    int s1Length = 0;
    memset(reserve,-1,sizeof(reserve));//把符号栈全部用—1替换
